...
#include "codegen/graph/gedge.hpp"
#include "codegen/graph/gnode.hpp"
#include "codegen/graph/graph.hpp"

#include "codegen/operators/op_define/fused.hpp"

using namespace std;
using namespace codegen::graph;
using namespace codegen::op;
                       ...
                       size_t output_size)
{
    ...
    {
        /*
        codegen_CHECK(input_gnodes.at(i)->get_output_size() == 1)
...
{
    codegen_CHECK(n >= m_inputs.size()) << "shrinking " << m_inputs.size() << " to " << n;
...
{
    codegen_CHECK(n >= m_outputs.size()) << "shrinking " << m_outputs.size() << " to " << n;
...
std::vector<std::shared_ptr<codegen::graph::Edge>> GNode::get_in_edges() const
{
    std::vector<std::shared_ptr<codegen::graph::Edge>> ret(m_in_edges.begin(), m_in_edges.end());
    std::sort(ret.begin(), ret.end(), EdgeComparatorDstIndex());
    return ret;
};

const std::shared_ptr<codegen::graph::Edge> GNode::get_in_edge(size_t i) const
{
    codegen_CHECK(i < m_inputs.size()) << "Input index " << i
                                        << " is out of range. GNode only has " << m_inputs.size()
                                        << " inputs.";
    std::shared_ptr<codegen::graph::Edge> found_in_edge = nullptr;
    for (auto in_edge : m_in_edges)
    {
        if (in_edge->get_dst_input() == i)
        {
            codegen_CHECK(found_in_edge == nullptr)
...
void GNode::remove_in_edge(std::shared_ptr<codegen::graph::Edge> edge)
{
    m_in_edges.erase(edge);
}

std::vector<std::shared_ptr<codegen::graph::Edge>> GNode::get_out_edges() const
{
    std::vector<std::shared_ptr<codegen::graph::Edge>> ret(m_out_edges.begin(), m_out_edges.end());
    std::sort(ret.begin(), ret.end(), EdgeComparatorSrcIndex());
    return ret;
};

std::vector<std::shared_ptr<codegen::graph::Edge>> GNode::get_output_users(size_t i)
{
    codegen_CHECK(i < m_outputs.size()) << "Output index " << i
                                         << " is out of range. GNode only has " << m_outputs.size()
                                         << " outputs.";
    std::vector<std::shared_ptr<codegen::graph::Edge>> output_users;
...
void GNode::add_out_edge(std::shared_ptr<codegen::graph::Edge> edge)
{
    m_out_edges.insert(edge);
}

void GNode::remove_out_edge(std::shared_ptr<codegen::graph::Edge> edge)
{
    m_out_edges.erase(edge);
}

codegen::descriptor::Tensor& GNode::get_input_tensor(size_t i) const
{
    auto in_edge = get_in_edge(i);
    return in_edge->get_src()->get_output_tensor(in_edge->get_src_output());
}

std::shared_ptr<codegen::descriptor::Tensor> GNode::get_input_tensor_ptr(size_t i) const
{
    auto in_edge = get_in_edge(i);
...
codegen::descriptor::Tensor& GNode::get_output_tensor(size_t i) const
{
    codegen_CHECK(i < m_outputs.size()) << "Output index " << i
                                         << " is out of range. GNode only has " << m_outputs.size()
                                         << " outputs.";
...
std::shared_ptr<codegen::descriptor::Tensor> GNode::get_output_tensor_ptr(size_t i) const
{
    codegen_CHECK(i < m_outputs.size()) << "Output index " << i
                                         << " is out of range. GNode only has " << m_outputs.size()
                                         << " outputs.";
...
void GNode::set_output_type_and_shape(size_t i,
                                      const codegen::element::Type& element_type,
                                      const codegen::PartialShape& pshape)
{
    if (i >= m_outputs.size())
    {
        set_output_size(i + 1);
...
const codegen::Shape& GNode::get_shape() const
{
    codegen_CHECK(get_output_size() == 1)
        << "get_shape() must be called on a node with exactly one output.";
    return m_outputs.at(0)->get_shape();
}

const codegen::element::Type& GNode::get_element_type() const
{
    codegen_CHECK(get_output_size() == 1)
...
const codegen::element::Type& GNode::get_output_element_type(size_t i) const
{
    return m_outputs.at(i)->get_element_type();
}

const codegen::Shape& GNode::get_output_shape(size_t i) const
{
    return m_outputs.at(i)->get_shape();
}

const codegen::PartialShape& GNode::get_output_partial_shape(size_t i) const
{
    return m_outputs.at(i)->get_partial_shape();
}

const codegen::element::Type& GNode::get_input_element_type(size_t i) const
{
    return m_inputs.at(i)->get_element_type();
}

const codegen::Shape& GNode::get_input_shape(size_t i) const
{
    return m_inputs.at(i)->get_shape();
}

const codegen::PartialShape& GNode::get_input_partial_shape(size_t i) const
{
...
const codegen::Shape& GNodeIndex::get_shape() const
{
    return gnode->get_output_shape(index);
}

const codegen::element::Type& GNodeIndex::get_element_type() const
{
...
void FusedGNode::reorder_nodes(std::unordered_set<std::shared_ptr<GNode>> nodes,
                               std::shared_ptr<Graph> graph)
{
    codegen_CHECK(!m_order_nodes.size());
...
{
    static_pointer_cast<codegen::op::Fused>(m_op_ptr)->register_ir2(m_order_nodes,
                                                                     shared_from_this());
}
