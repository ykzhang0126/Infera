...
{
    codegen_CHECK(m_name.empty()) << "Graph name may be set exactly once.";
...
void Graph::add_node(std::shared_ptr<GNode> node)
{
    if (node->get_id() != freeGnodeId)
    {
        ...
        codegen_CHECK(m_nodes[node->get_id()] == node);
        return;
    }
...
std::shared_ptr<GNode> Graph::add_node_and_edge(const std::shared_ptr<codegen::op::Op> op,
...
void Graph::add_gnode_and_edge(const std::shared_ptr<GNode> gnode,
                               const GNodeIndexVector& input_gnodes)
{
    codegen_CHECK(gnode == nullptr) << "GNode can't be nullptr.";
...
void Graph::remove_node(std::shared_ptr<GNode> node)
{
    ...
    //codegen_DCHECK(!node->IsSource());
    //codegen_DCHECK(!node->IsSink());
...
const std::shared_ptr<codegen::graph::Edge>
    Graph::add_edge(std::shared_ptr<GNode> source, int x, std::shared_ptr<GNode> dest, int y)
{
    ...
    else
    {
        ...
        codegen_CHECK(source_shape.compatible(dest_shape))
...
const std::shared_ptr<codegen::graph::Edge> Graph::add_control_edge(std::shared_ptr<GNode> source,
...
void Graph::remove_edge(std::shared_ptr<Edge> edge)
{
    ...
    codegen_CHECK(edge == m_edges[edge->m_id]);
...
{
    codegen::serialize::GraphDef graphdef;
    auto codegen_nodes = get_ordered_ops();
    for (auto& codegen_node : codegen_nodes)
    {
        codegen_CHECK(
            !codegen_node->hasAttributes() ||
            (codegen_node->attributeNames().size() == 1 && codegen_node->hasAttribute("Alias")))
            << codegen_node->get_name() << " has " << codegen_node->attributeNames().size()
            << " tags including \"Alias\" which cannot be serialized now.";
        codegen::serialize::NodeDef* node = graphdef.add_node();
        // name
        node->set_name(codegen_node->get_name());
        // op
        node->set_op(codegen_node->get_op_type());
        // input
        for (auto codegen_edge : codegen_node->get_in_edges())
        {
            if (codegen_edge->get_src_output() == kControlSlot)
            {
                node->add_input("^" + codegen_edge->get_src()->get_name());
            }
            else
            {
                node->add_input(codegen_edge->get_src()->get_name() + ":" +
                                std::to_string(codegen_edge->get_src_output()));
            }
        }
        // TODO(gbxu): support all codegen ops
        if (codegen_node->get_op_type() == "AllReduce")
        {
            // tensor_name
            codegen::serialize::AttrValue tensor_name;
            tensor_name.set_s(codegen_node->get_name());
            (*node->mutable_attr())["tensor_name"] = tensor_name;
        }
        // data type
        if (codegen_node->get_output_size() == 1)
        {
            codegen::serialize::AttrValue data_type;
            codegen::serialize::PBType dt;
            codegen::element::Type::codegen_element_type_to_pbtype(
                codegen_node->get_element_type(), dt);
        ...
        {            
            codegen::serialize::AttrValue_ListValue* _data_types_list =
                new codegen::serialize::AttrValue_ListValue();
            for (auto codegen_output : codegen_node->get_outputs())
            {
                codegen::serialize::PBType dt;
                codegen::element::Type::codegen_element_type_to_pbtype(
                    codegen_output->get_element_type(), dt);
                _data_types_list->add_type(dt);
            }
            codegen::serialize::AttrValue _data_types;
            _data_types.set_allocated_list(_data_types_list);
            (*node->mutable_attr())["T"] = _data_types;
        }
#endif
        // _output_shapes
        codegen::serialize::AttrValue_ListValue* _output_shapes_list =
            new codegen::serialize::AttrValue_ListValue();
        for (auto codegen_output : codegen_node->get_outputs())
        {
            auto shape = _output_shapes_list->add_shape();
            for (auto codegen_dim : codegen_output->get_shape())
            {
                auto dim = shape->add_dim();
                dim->set_size(codegen_dim);
            }
        }
        codegen::serialize::AttrValue _output_shapes;
