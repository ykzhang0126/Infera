...
def launch_jobs(config_dict_list, input_args, run_eval):
    ...
    home_directory = os.path.expanduser( '~' )
    if run_eval:
        sched_lib = cdll.LoadLibrary(home_directory + "/infera/src/scheduler/scheduler_eval.so")
    else:
        sched_lib = cdll.LoadLibrary(home_directory + "/infera/src/scheduler/scheduler.so")
    ...
    sched_thread = threading.Thread(
        ...
        args=(
            ...
            input_args.reef_depth if input_args.algo=='reef' else input_args.infera_max_be_duration,
            input_args.infera_hp_limit,
            input_args.infera_start_update,
            train_list
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--algo', type=str, required=True,
                        help='choose one of infera | reef | sequential')
    parser.add_argument('--config_file', type=str, required=True,
                        help='path to the experiment configuration file')
    parser.add_argument('--reef_depth', type=int, default=1,
                        help='If reef is used, this stands for the queue depth')
    parser.add_argument('--infera_max_be_duration', type=int, default=1,
                        help='If infera is used, the maximum aggregate duration of on-the-fly best-effort kernels')
    parser.add_argument('--infera_start_update', type=int, default=1,
                        help='If infera is used, and the high priority job is training, this is the kernel id after which the update phase starts')
    parser.add_argument('--infera_hp_limit', type=int, default=1,
                        help='If infera is used, and the high priority job is training, this shows the maximum tolerated training iteration time')
